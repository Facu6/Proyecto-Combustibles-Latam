...
def subir_csv_a_gcs(project_id, bucket_name, archivo_local, destino_gcs):
    ...
    try:    
        ...
        logging.info(f'Archivo de Uruguay subido a GCS: "gs://{bucket_name}/{destino_gcs}"')
    
    except Exception as e:
        logging.error(f'Error al cargar el archivo de Uruguay a GCS: {e}')
...
def crear_tabla_bigquery(project_id, dataset_id, table_id, schema):
    ...
    try: 
        ...
        logging.info(f'Tabla de Uruguay creada en BigQuery: {table_id}.')
    except Conflict:
        logging.info(f'La tabla de Uruguay ya existe: {table_id}.')
...
def cargar_datos_tabla(project_id, dataset_id, table_id, gcs_uri):
    ...
    try:
        ...
        logging.info(f'Datos de Uruguay cargados en BigQuery: {table_id}')
    
    except Exception as e:
        logging.error(f'Error al cargar datos de Uruguay en tabla {table_id}: {e}')

def ejecutar_carga_uruguay():
    
    logging.info('=== Iniciando carga de datos de Uruguay ===')
    # Ruta local del archivo procesado
    ruta_local = os.path.join(DATA_PROCESSED_DIR, UR_DATASET_FILENAME)
    
    # Ruta destino dentro del bucket
    destino_gsc = f'processed_data/uruguay/{UR_DATASET_FILENAME}'
    ...
    crear_tabla_bigquery(PROJECT_ID, BQ_DATASET_LATAM, BQ_TABLE_UR, schema)
    
    # Cargar datos a BigQuery
    cargar_datos_tabla(PROJECT_ID, BQ_DATASET_LATAM, BQ_TABLE_UR, gcs_uri)
