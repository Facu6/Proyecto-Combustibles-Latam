...
def subir_csv_a_gcs(project_id, bucket_name, archivo_local, destino_gcs):
    ...
    try:    
        ...
        logging.info(f'Archivo de Argentina subido a GCS: "gs://{bucket_name}/{destino_gcs}"')
    
    except Exception as e:
        logging.error(f'Error al cargar el archivo de Argentina a GCS: {e}')
...
def crear_tabla_bigquery(project_id, dataset_id, table_id, schema):
    ...
    try: 
        ...
        logging.info(f'Tabla de Argentina creada en BigQuery: {table_id}.')
    except Conflict:
        logging.info(f'La tabla de Argentina ya existe: {table_id}.')
...
def cargar_datos_tabla(project_id, dataset_id, table_id, gcs_uri):
    ...
    try:
        ...
        logging.info(f'Datos de Argentina cargado en BigQuery: {table_id}')
    
    except Exception as e:
        logging.error(f'Error al cargar datos de Argentina en tabla {table_id}: {e}')

def ejecutar_carga_argentina():
    
    logging.info('=== Iniciando carga de datos de Argentina ===')
    # Ruta local del archivo procesado
    ruta_local = os.path.join(DATA_PROCESSED_DIR, AR_DATASET_FILENAME)
    
    # Ruta destino dentro del bucket
    destino_gsc = f'processed_data/argentina/{AR_DATASET_FILENAME}'
    ...
    crear_tabla_bigquery(PROJECT_ID, BQ_DATASET_LATAM, BQ_TABLE_AR, schema)
    
    # Cargar datos a BigQuery
    cargar_datos_tabla(PROJECT_ID, BQ_DATASET_LATAM, BQ_TABLE_AR, gcs_uri)
